...
-- Pipeline setup remains editable until its version is published. Publishing is
-- one-way; a published version and all of its stages are then immutable.
...
-- A task is assembled while unlocked. Locking is one-way and freezes its
-- pipeline assignment and structural stage/checklist snapshot.
