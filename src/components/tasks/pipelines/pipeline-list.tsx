  ...
  return (
    <section aria-labelledby="pipelines-heading" className="space-y-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div><h1 id="pipelines-heading" className="text-xl font-semibold text-text-primary">Pipelines</h1><p className="mt-1 text-sm text-text-secondary">Shared stage definitions for future tasks.</p></div>
        <Button leftIcon={<Plus />} onClick={onCreate} aria-label="Create pipeline">Create pipeline</Button>
      </div>
      {pipelines.length === 0 ? (
        <div className="card p-6 sm:p-12 text-center"><Workflow className="mx-auto mb-3 h-10 w-10 text-text-muted" aria-hidden="true" /><h2 className="text-base font-semibold text-text-primary">No pipelines yet</h2><p className="mt-1 text-sm text-text-secondary">Create a reusable set of task stages to get started.</p></div>
      ) : (
        <div className="grid grid-cols-1 gap-3 lg:grid-cols-2">
          {pipelines.map((pipeline) => {
            const activeVersion = pipeline.versions[0];
            return <article key={pipeline.id} className="card flex min-w-0 flex-col gap-3 p-4">
              <div className="flex items-start gap-3"><Workflow className="mt-0.5 h-4 w-4 shrink-0 text-text-secondary" aria-hidden="true" /><div className="min-w-0 flex-1"><h2 className="truncate text-base font-semibold text-text-primary">{pipeline.name}</h2><p className="mt-1 line-clamp-2 text-sm text-text-secondary">{pipeline.description || 'No description'}</p></div></div>
