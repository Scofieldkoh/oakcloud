...
export function DeadlineRulesPanel({
  ...
  const list = useDeadlineRules(workspaceId, filters);
  const firstId = list.data?.rules[0]?.id;
  const activeId = selectedId ?? firstId;
  const detail = useDeadlineRule(workspaceId, activeId);
  const selected = detail.data ?? list.data?.rules.find((rule) => rule.id === activeId) ?? null;
  ...
  if (creating) {
    return <section aria-labelledby="deadline-rules-heading"><div className="mb-4"><h2 id="deadline-rules-heading" className="text-lg font-semibold text-text-primary">Deadline rules</h2><p className="mt-1 text-sm text-text-secondary">Create a versioned rule set for any service family.</p></div><div className="rounded-lg border border-border-primary bg-background-secondary"><DeadlineRuleForm onCancel={() => setCreating(false)} onSubmit={handleDraftSubmit} onDirtyChange={() => setPreview(null)} isSubmitting={create.isPending} /></div></section>;
  }
  ...
  return (
    <section aria-labelledby="deadline-rules-heading">
      <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        ...
        <Button type="button" size="sm" className="min-h-[44px] sm:min-h-8" leftIcon={<Plus className="h-4 w-4" />} onClick={() => { setCreating(true); setPreview(null); }}>Add deadline rule</Button>
      </div>
      ...
      <div className="mt-4 grid grid-cols-1 gap-4 lg:grid-cols-[minmax(220px,0.35fr)_minmax(0,1fr)]">
        ...
        <div className="min-w-0 rounded-lg border border-border-primary bg-background-secondary">
          {detail.isLoading && activeId ? <div role="status" className="flex items-center gap-2 p-8 text-sm text-text-muted"><Loader2 className="h-4 w-4 animate-spin" /> Loading rule detail…</div> : detail.error ? <div className="space-y-2 p-8"><p className="font-medium text-status-error">Unable to load rule detail.</p><Button type="button" variant="secondary" className="min-h-[44px]" onClick={() => void detail.refetch()}>Retry</Button></div> : selected ? <>
            <div className="flex flex-col gap-3 border-b border-border-primary p-4 sm:flex-row sm:items-start sm:justify-between"><div><h3 className="flex flex-wrap items-center gap-2 text-lg font-semibold text-text-primary">{selected.name}<span className={selected.isActive ? 'badge badge-success' : 'badge badge-neutral'}>{selected.isActive ? `Active v${selected.currentVersion?.version ?? 'draft'}` : 'Inactive'}</span></h3><p className="mt-1 text-sm text-text-secondary">{selected.description || selected.code}</p><p className="mt-1 text-xs text-text-muted">Published {formatDate(selected.currentVersion?.publishedAt ?? null)} · Draft revision {selected.draft?.draftRevision ?? '—'}</p></div><div className="flex flex-wrap gap-2"><Button type="button" variant="secondary" size="xs" className="min-h-[44px]" aria-label={`Edit ${selected.name}`} onClick={() => { setEditing(true); setPreview(null); }}>Edit rule</Button><Button type="button" variant="ghost" size="xs" className="min-h-[44px]" onClick={() => void handlePreview('PUBLISH')} disabled={previewMutation.isPending}>Preview impact</Button><Button type="button" variant="ghost" size="xs" className="min-h-[44px]" onClick={() => void handlePreview('ARCHIVE')} disabled={previewMutation.isPending}><Archive className="h-3.5 w-3.5" /> Preview archive impact</Button></div></div>
            {editing ? <><div className="border-b border-border-primary"><DeadlineRuleForm initialValue={selected} onCancel={() => setEditing(false)} onSubmit={handleDraftSubmit} onDirtyChange={() => { setPreview(null); setArchivePreview(null); }} isSubmitting={update.isPending} /></div><div className="flex justify-end border-b border-border-primary p-4"><Button type="button" className="min-h-[44px]" onClick={handlePublish} disabled={!preview || preview.operation !== 'PUBLISH' || publish.isPending} isLoading={publish.isPending}>Publish rule</Button></div></> : <>
              ...
              <div className="flex flex-wrap items-center justify-end gap-2 border-t border-border-primary p-4"><Button type="button" className="min-h-[44px]" onClick={handlePublish} disabled={!preview || preview.operation !== 'PUBLISH' || publish.isPending} isLoading={publish.isPending}>Publish rule</Button><Button type="button" variant="danger" className="min-h-[44px]" onClick={() => { if (archivePreview) setArchiveDialogOpen(true); else void handlePreview('ARCHIVE'); }} disabled={archive.isPending}>Archive rule</Button></div>
            </>}
          </> : <div className="p-8 text-center text-sm text-text-muted">Select a deadline rule to inspect its versioned configuration.</div>}
        </div>
      </div>

      <RuleImpactDialog impact={preview} onClose={() => setPreview(null)} />
      <RuleImpactDialog impact={archivePreview} onClose={() => setArchivePreview(null)} />
      <Modal isOpen={archiveDialogOpen} onClose={() => { if (!archive.isPending) setArchiveDialogOpen(false); }} title="Archive deadline rule" description="Archiving preserves history and cancels only eligible future open occurrences.">
        <div className="space-y-4 p-4"><FormInput label="Archive reason" value={archiveReason} onChange={(event) => setArchiveReason(event.target.value)} hint="At least 10 characters are required for the audit trail." disabled={archive.isPending} className="min-h-[44px]" /><p className="text-xs text-text-muted">Current archive impact fingerprint: {archivePreview?.previewFingerprint.slice(0, 12)}…</p><div className="flex justify-end gap-2"><Button type="button" variant="secondary" className="min-h-[44px]" onClick={() => setArchiveDialogOpen(false)} disabled={archive.isPending}>Cancel</Button><Button type="button" variant="danger" className="min-h-[44px]" onClick={() => void handleArchive()} disabled={archiveReason.trim().length < 10 || !archivePreview} isLoading={archive.isPending}>Archive rule</Button></div></div>
      </Modal>
    </section>
  );
}
