...
export function DeadlineRulesPanel({
  ...
  if (creating) {
    return <section aria-labelledby="deadline-rules-heading"><div className="mb-4 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between"><div><h2 id="deadline-rules-heading" className="text-lg font-semibold text-text-primary">Deadline rules</h2><p className="mt-1 text-sm text-text-secondary">Create a versioned rule set for any service family.</p></div></div><div className="rounded-lg border border-border-primary bg-background-secondary"><DeadlineRuleForm onCancel={() => { setCreating(false); setDraftDirty(false); }} onSubmit={handleDraftSubmit} onDirtyChange={() => { setDraftDirty(true); setPreview(null); setArchivePreview(null); }} isSubmitting={create.isPending} /></div></section>;
  }
  ...
  return (
    <section aria-labelledby="deadline-rules-heading">
      <div className="mb-4 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-3"><Button type="button" size="sm" className="min-h-11 sm:min-h-8" leftIcon={<Plus className="h-4 w-4" />} onClick={() => { setCreating(true); setDraftDirty(false); setPreview(null); }}>Add deadline rule</Button></div>
      </div>
      {actionError ? <Alert variant="error" title="Action failed" onClose={() => setActionError(null)}>{actionError}</Alert> : null}
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-[minmax(220px,0.35fr)_minmax(0,1fr)]">
        <aside className="rounded-lg border border-border-primary bg-background-secondary" aria-label="Deadline rule list">
          <div className="grid gap-3 border-b border-border-primary p-4">
            <FormInput aria-label="Search deadline rules" type="search" value={query} onChange={(event) => { setQuery(event.target.value); setPage(1); }} placeholder="Search code or name" leftIcon={<Search className="h-4 w-4" />} className="input input-sm min-h-11 sm:min-h-8" />
            <label className="block text-xs font-medium text-text-secondary">Status<select aria-label="Deadline rule status" className="input input-sm mt-2 min-h-11 w-full sm:min-h-8" value={status} onChange={(event) => { setStatus(event.target.value as StatusFilter); setPage(1); }}><option value="all">All</option><option value="active">Active</option><option value="inactive">Inactive / archived</option></select></label>
          </div>
           {list.isLoading ? <div role="status" className="flex items-center gap-2 p-4 text-sm text-text-muted"><Loader2 className="h-4 w-4 animate-spin" /> Loading deadline rules…</div> : list.error ? <div className="space-y-2 p-4 text-sm"><p className="font-medium text-status-error">Unable to load deadline rules.</p><p className="text-text-muted">{list.error instanceof Error ? list.error.message : 'Try again shortly.'}</p><Button type="button" variant="secondary" className="min-h-11 sm:min-h-8" onClick={() => void list.refetch()}>Retry</Button></div> : list.data?.rules.length ? <div className="divide-y divide-border-secondary">{list.data.rules.map((rule) => <button key={rule.id} type="button" className={`flex min-h-11 w-full flex-col items-start gap-1 px-3 py-3 text-left transition-colors hover:bg-background-tertiary focus:outline-none focus-visible:ring-2 focus-visible:ring-oak-primary/30 sm:min-h-8 ${rule.id === activeId ? 'border-l-2 border-oak-primary bg-background-tertiary' : ''}`} aria-pressed={rule.id === activeId} onClick={() => { setSelectedId(rule.id); setSelectedOverride(null); setEditing(false); setDraftDirty(false); setPreview(null); setArchivePreview(null); setArchiveImpactOpen(false); }}><span className="flex flex-wrap items-center gap-2 font-medium text-text-primary">{rule.name}<span className={rule.isActive ? 'badge badge-success' : 'badge badge-neutral'}>{rule.isActive ? 'Active' : 'Inactive'}</span></span><span className="text-xs text-text-muted">{rule.code}{rule.currentVersion ? ` · Published v${rule.currentVersion.version}` : ' · Draft only'}</span></button>)}</div> : <div className="p-4 text-center"><p className="text-sm font-medium text-text-primary">No deadline rules found</p><p className="mt-1 text-xs text-text-muted">Create a rule or adjust the search and status filters.</p></div>}
        ...
        <div className="min-w-0 rounded-lg border border-border-primary bg-background-secondary">
           {detail.isLoading && activeId ? <div role="status" className="flex items-center gap-2 p-4 text-sm text-text-muted"><Loader2 className="h-4 w-4 animate-spin" /> Loading rule detail…</div> : detail.error ? <div className="space-y-2 p-4"><p className="font-medium text-status-error">Unable to load rule detail.</p><Button type="button" variant="secondary" className="min-h-11 sm:min-h-8" onClick={() => void detail.refetch()}>Retry</Button></div> : selected ? <>
             <div className="flex flex-col gap-4 border-b border-border-primary p-4 sm:flex-row sm:items-start sm:justify-between"><div><h3 className="flex flex-wrap items-center gap-2 text-lg font-semibold text-text-primary">{selected.name}<span className={selected.isActive ? 'badge badge-success' : 'badge badge-neutral'}>{selected.isActive ? `Active v${selected.currentVersion?.version ?? 'draft'}` : 'Inactive'}</span></h3><p className="mt-1 text-sm text-text-secondary">{selected.description || selected.code}</p><p className="mt-1 text-xs text-text-muted">Published {formatDate(selected.currentVersion?.publishedAt ?? null)} · Draft revision {selected.draft?.draftRevision ?? '—'}</p></div><div className="flex flex-wrap items-center gap-3"><Button type="button" variant="secondary" size="xs" className="min-h-11 sm:min-h-8" aria-label={`Edit ${selected.name}`} onClick={() => { setEditing(true); setDraftDirty(false); setPreview(null); setArchivePreview(null); }}>Edit rule</Button><Button type="button" variant="ghost" size="xs" className="min-h-11 sm:min-h-8" onClick={() => void handlePreview('PUBLISH')} disabled={previewMutation.isPending || draftDirty}>Preview impact</Button><Button type="button" variant="ghost" size="xs" className="min-h-11 sm:min-h-8" onClick={() => void handlePreview('ARCHIVE')} disabled={previewMutation.isPending || draftDirty}><Archive className="h-3.5 w-3.5" /> Preview archive impact</Button></div></div>
             {editing ? <><div className="border-b border-border-primary"><DeadlineRuleForm initialValue={selected} onCancel={() => { setEditing(false); setDraftDirty(false); }} onSubmit={handleDraftSubmit} onDirtyChange={() => { setDraftDirty(true); setPreview(null); setArchivePreview(null); setArchiveImpactOpen(false); }} isSubmitting={update.isPending} /></div><div className="flex justify-end border-b border-border-primary p-4"><Button type="button" className="min-h-11 sm:min-h-8" onClick={handlePublish} disabled={draftDirty || !preview || preview.operation !== 'PUBLISH' || publish.isPending} isLoading={publish.isPending}>Publish rule</Button></div></> : <>
              ...
              <div className="flex flex-wrap items-center justify-end gap-3 border-t border-border-primary p-4"><Button type="button" className="min-h-11 sm:min-h-8" onClick={handlePublish} disabled={draftDirty || !preview || preview.operation !== 'PUBLISH' || publish.isPending} isLoading={publish.isPending}>Publish rule</Button><Button type="button" variant="danger" className="min-h-11 sm:min-h-8" onClick={() => { if (archivePreview) { setArchiveImpactOpen(true); } else void handlePreview('ARCHIVE'); }} disabled={archive.isPending || draftDirty}>Archive rule</Button></div>
            </>}
      ...
      <Modal isOpen={archiveDialogOpen} onClose={clearArchiveFlow} title="Archive deadline rule" description="Archiving preserves history and cancels only eligible future open occurrences.">
        <div className="space-y-4 p-4"><FormInput label="Archive reason" value={archiveReason} onChange={(event) => setArchiveReason(event.target.value)} hint="At least 10 characters are required for the audit trail." disabled={archive.isPending} className="input input-sm min-h-11 sm:min-h-8" /><p className="text-xs text-text-muted">Current archive impact fingerprint: {archivePreview?.previewFingerprint.slice(0, 12)}…</p><div className="flex justify-end gap-3"><Button type="button" variant="secondary" className="min-h-11 sm:min-h-8" onClick={clearArchiveFlow} disabled={archive.isPending}>Cancel</Button><Button type="button" variant="danger" className="min-h-11 sm:min-h-8" onClick={() => void handleArchive()} disabled={archiveReason.trim().length < 10 || !archivePreview} isLoading={archive.isPending}>Archive rule</Button></div></div>
      </Modal>
    </section>
  );
}
