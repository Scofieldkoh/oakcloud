...
import {
  ...
  type TemplateEditorPartialForm,
  type TemplateEditorTemplateForm,
...
export function TemplateEditorPanel({ mode, templateForm, partialForm, onTemplateChange, onPartialChange, fieldsContent, testPreviewContent, aiContent, validationIssues, onFocusIssue, isDirty = false, panel, isSuperAdmin, activeTenantId, tenantName, dateFields = [] }: TemplateEditorPanelProps) {
  ...
  const tabs: Array<{ id: PanelTab; label: string }> = [
    ...
    { id: 'test-preview', label: 'Test & Preview' },
  ];
  ...
  return (
    <aside aria-label="Template editor tools" style={panelStyle} className={`relative flex h-full shrink-0 border-l border-border-primary bg-background-secondary transition-[width] duration-150 ${collapsed ? 'w-10' : 'min-w-80'}`}>
      <button type="button" aria-label={collapsed ? 'Expand editor tools' : 'Collapse editor tools'} onClick={panel?.toggle} className="absolute -left-8 top-3 z-10 flex h-8 w-8 items-center justify-center rounded-l-md border border-border-primary bg-background-secondary text-text-secondary transition-colors duration-150 hover:bg-background-tertiary">
        {collapsed ? <ChevronLeft className="h-4 w-4" /> : <ChevronRight className="h-4 w-4" />}
      </button>
      {!collapsed && (
        <>
          <div role="separator" aria-orientation="vertical" aria-label="Resize editor tools" onMouseDown={(event) => panel?.startResize(event, 'left')} className={`absolute inset-y-0 -left-1 w-2 cursor-col-resize ${panel?.isResizing ? 'bg-accent-primary/30' : ''}`} />
          <div className="flex min-h-0 flex-1 flex-col">
            <div className="flex items-center justify-between border-b border-border-primary px-3 py-2">
              <span className="text-xs font-semibold text-text-primary">Editor tools</span>
              {isDirty && <span className="rounded-full bg-status-warning/15 px-2 py-0.5 text-[11px] font-medium text-status-warning">Unsaved changes</span>}
            </div>
            <div role="tablist" aria-label="Template editor tasks" className="flex border-b border-border-primary">
