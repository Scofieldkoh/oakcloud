...
export function CompanyServicesTab({ companyId, canEdit }: { companyId: string; canEdit: boolean }) {
  ...
  return <div className="space-y-4">
    ...
    {services.length === 0 ? <div className="card p-6 text-center sm:p-10"><BriefcaseBusiness className="mx-auto mb-3 h-10 w-10 text-text-muted" /><h3 className="font-medium text-text-primary">{filtered ? 'No matching services' : 'No services yet'}</h3><p className="mt-1 text-sm text-text-secondary">{filtered ? 'Try adjusting your search or status filter.' : 'Services appear here when they are added manually or activated from a Service Agreement.'}</p>{!filtered && canEdit ? <Button className="mt-4 min-h-11 sm:min-h-8" size="sm" variant="secondary" leftIcon={<Plus />} onClick={() => setCreating(true)}>Add service</Button> : null}</div> : <div className="space-y-3">{services.map((service) => {
      ...
      ) : null}</div>{canEdit ? <Button className="min-h-11 sm:min-h-8" size="xs" variant="secondary" leftIcon={<Pencil />} onClick={() => setEditing(service)} aria-label="Edit service">Edit</Button> : null}</div></article>;
    })}</div>}
