...
export async function POST(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    const input = deadlineRuleImpactPreviewSchema.parse(payload);
    const impact = await previewDeadlineRuleImpact(id, input, { tenantId, userId: session.id });
    return NextResponse.json(impact);
  } catch (error) {
    return serviceCatalogErrorResponse(error, 'POST /api/service-catalog/deadline-rules/[id]/impact');
  }
}
