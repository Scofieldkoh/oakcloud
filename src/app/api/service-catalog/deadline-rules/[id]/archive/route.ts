...
export async function POST(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    const input = deadlineRuleArchiveSchema.parse(payload);
    const rule = await archiveDeadlineRule(id, input, { tenantId, userId: session.id });
    return NextResponse.json(rule);
  } catch (error) {
    return serviceCatalogErrorResponse(error, 'POST /api/service-catalog/deadline-rules/[id]/archive');
  }
}
