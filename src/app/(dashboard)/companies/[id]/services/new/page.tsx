...
import { useCreateCompanyService } from '@/hooks/use-contract-services';
...
export default function NewServicePage({ params, searchParams }: PageProps) {
  ...
  const createServiceMutation = useCreateCompanyService(companyId);
  ...
  const templateOptions = useMemo(() => {
    return templateCatalog.map((template) => ({
      ...
      group: TEMPLATE_CATEGORY_LABELS[template.category] || template.category,
    }));
  }, [templateCatalog]);
  ...
  // Handle template selection with overwrite protection for in-progress drafts.
  ...
  type TemplatePayload = ReturnType<typeof buildTemplatePayload>;

  const applyTemplatePayload = useCallback((payload: TemplatePayload, templateCode: string | null) => {
    ...
    setValue('scope', payload.scope ?? '');
    setDeadlineRules(payload.deadlineRules.map((rule, index) => ({
      ...rule,
      displayOrder: index,
    })));
  }, [setValue]);
  ...
  const handleSaveTemplate = useCallback(async () => {
    ...
    try {
      ...
      if (templateEditorMode === 'create') {
        ...
        nextCode = overridden.code;
      }

      if (nextCode) {
        applyTemplatePayload(payload, nextCode);
      }
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      {/* Form */}
      <form onSubmit={handleSubmit(onSubmit)} aria-label="Add new service form" className="flex-1 flex flex-col px-4 sm:px-6 pb-4 sm:pb-6 pt-0 overflow-auto">
