...
/**
 * Deliberate F0 baseline failures. They freeze compatibility gaps for G0 and
 * must stay `it.fails` until F1/F2 is explicitly dispatched. F0 does not turn
 * on new parsing, escaping, scoping or serialization behavior.
 */
describe('FIELDS F0 compatibility baseline', () => {
  it.fails('baseline: spaced simple syntax accepted by editor validation also resolves', () => {
    const source = '<p>{{ custom.note }}</p>';
    const issues = validateEditorTemplateSyntax(source, new Set(['custom.note']));
    expect(issues).toEqual([]);

    const result = resolvePlaceholders(source, { custom: { note: 'Resolved note' } });
    expect(result.resolved).toBe('<p>Resolved note</p>');
    expect(result.missing).toEqual([]);
  });

  it.fails('baseline: formatting inside a field expression is diagnosed', () => {
    const source = '<p>{{custom.<b>note</b>}}</p>';
    const issues = validateEditorTemplateSyntax(source, new Set(['custom.note']));
    expect(issues).toEqual([
      expect.objectContaining({ code: 'unknown-placeholder' }),
    ]);
  });

  it.fails('baseline: a no-change editor round-trip preserves legacy list type', () => {
    const stored = [{
      key: 'custom.items',
      label: 'Items',
      type: 'list',
    ...
    const roundTrip = editorPlaceholdersToStorage(storagePlaceholdersToEditor(stored));
    expect(roundTrip).toEqual(stored);
  });

  it.fails('baseline: a no-change editor round-trip preserves legacy conditional type', () => {
    const stored = [{
      key: 'custom.enabled',
      label: 'Enabled',
      type: 'conditional',
  ...
  it.fails('baseline: loading the same stored field twice yields the same identity', () => {
    const stored = [{
      key: 'custom.note',
      label: 'Note',
      type: 'text',
  ...
  it.fails('baseline: the two field adapters agree on an omitted required flag', () => {
    const stored = [{
      key: 'custom.note',
      label: 'Note',
      type: 'text',
  ...
  it.fails('baseline: parent and partial collisions retain the same scoped key instead of renaming input identity', () => {
  ...
  it.fails('baseline: nested partial field discovery includes nested definitions', () => {
    const merged = mergeTemplateAndPartialPlaceholders({
      templatePlaceholders: [],
  ...
  it.fails('baseline: ordinary text values cannot introduce HTML markup', () => {
  ...
  it.fails('baseline: ISO-looking text remains text unless its field type says date', () => {
    const result = resolvePlaceholders(
