...
const contacts = [
  {
    ...
    referenceCounts: { companyRelations: 1, officerPositions: 1, shareholdings: 0, chargeHoldings: 0, contactDetails: 1, noteTabs: 1, workflowCommunicationLogEntries: 0, workflowMilestones: 0 },
  ...
  },
  {
    ...
    referenceCounts: { companyRelations: 0, officerPositions: 0, shareholdings: 1, chargeHoldings: 0, contactDetails: 1, noteTabs: 0, workflowCommunicationLogEntries: 1, workflowMilestones: 0 },
  ...
  },
  {
    ...
    corporateName: null, alias: null, identificationType: null, identificationNumber: null,
    corporateUen: null, nationality: null, dateOfBirth: null, fullAddress: null,
    contactDetails: [], companies: [],
    referenceCounts: { companyRelations: 0, officerPositions: 0, shareholdings: 0, chargeHoldings: 0, contactDetails: 0, noteTabs: 0, workflowCommunicationLogEntries: 0, workflowMilestones: 1 },
...
vi.mock('@/hooks/use-contacts', () => ({
  useContactDuplicateGroups: () => ({ data: queryData, isLoading: false, isFetching: false, error: null, refetch: mocks.refetch }),
  useRejectContactDuplicate: () => ({ mutateAsync: mocks.reject, isPending: false }),
...
describe('ContactDuplicateReviewModal', () => {
  ...
  it('requires conflict resolution and warns that sources are permanently deleted', async () => {
    ...
    const confirmation = (await screen.findByText('Permanently merge contacts')).closest<HTMLElement>('[role="dialog"]');
  ...
  it('requires a rejection reason before rejecting the recommendation', async () => {
    ...
    await waitFor(() => expect(mocks.reject).toHaveBeenCalledWith(expect.objectContaining({
      leftContactId: ids.recommended,
      rightContactId: ids.alternate,
      reason: 'These are different people',
    })));
  });
