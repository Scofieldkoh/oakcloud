...
describe('TemplatesPage service tab', () => {
  it('restores the Services tab from the URL', async () => {
    vi.resetModules();
    ...
    vi.doMock('@/hooks/use-permissions', () => ({
      usePermissions: () => ({
        can: {
          createDocument: true,
          updateDocument: true,
    ...
    vi.doMock('@/components/ui/workspace-selector', () => ({
      useActiveWorkspaceId: () => 'tenant-1',
    }));
    vi.doMock('@/components/documents/document-generation-tabs', () => ({
      DocumentGenerationTabs: () => null,
    }));
    vi.doMock(
      '@/components/documents/service-catalog/service-catalog-panel',
      () => ({
    ...
    const { default: TemplatesPage } = await import(
      '@/app/(dashboard)/template-partials/page'
    );
