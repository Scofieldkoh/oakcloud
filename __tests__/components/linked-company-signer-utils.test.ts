...
describe('linked company signer quick-add rules', () => {
  ...
  it('validates the email needed for an emailed signing request', () => {
    expect(isValidSignerEmail(' Jane@Example.com ')).toBe(true);
    expect(isValidSignerEmail('not-an-email')).toBe(false);
    expect(isValidSignerEmail(null)).toBe(false);
  });
  ...
  it('disables a company contact that has no usable default email', () => {
    const missing = getLinkedCompanyQuickAddState(
      { id: 'contact-1', fullName: 'No Email', defaultEmail: null },
      new Set(),
      null,
    );
    const invalid = getLinkedCompanyQuickAddState(
      { id: 'contact-2', fullName: 'Bad Email', defaultEmail: 'invalid' },
      new Set(),
      null,
    );

    expect(missing.isDisabled).toBe(true);
    expect(invalid.isDisabled).toBe(true);
    expect(missing.stateLabel).toContain('valid default email');
  });
  ...
  it('does not build a signer payload without a valid name and email', () => {
    expect(buildLinkedCompanySignerInput({
      id: 'contact-1',
      fullName: 'Jane Example',
      defaultEmail: null,
  ...
  it('filters candidates to contacts that can become new signers', () => {
    const contacts = [
  ...
  it('deduplicates candidates by normalized email and keeps the first contact', () => {
    const contacts = [
